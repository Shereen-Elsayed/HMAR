...
tstInt = None
with open('./Datasets/Yelp_tst_int', 'rb') as fs:
...
class Model():
    def __init__(self, usernum, itemnum, args, reuse=None):
        ...
        self.classification_pred = tf.compat.v1.layers.dense(inputs=classification_pred1 , units=4 ,activation=None, reuse=False, kernel_initializer=tf.random_normal_initializer(stddev=0.01) , kernel_regularizer = tf.keras.regularizers.l2(args.l2_emb_class), name="classification")
...
def sample_function(user_train, Beh, Beh_w, Behaviors, usernum, itemnum, batch_size, maxlen, result_queue, SEED):
    def sample():
        ...
        Aug_seq, Aug_seq_cxt, Aug_buy_seq_mask, Aug_cart_seq_mask, Aug_fav_seq_mask, Aug_click_seq_mask  = Augmented_seq(user_train[user][:-1],Beh[user][:-1], Behaviors, Beh_w[user][:-1],maxlen)
...
'''
maxlen... 170    heads.... 1    dropout... 0.4   lr,... 0.0007  emb size... 50 min len.. 20  L2 Reg.. 0.0 Seed 42 final best
maxlen... 170    heads.... 1    dropout... 0.4   lr,... 0.0007  emb size... 55 min len.. 20 
maxlen... 200    heads.... 1    dropout... 0.4   lr,... 0.0007  emb size... 50 min len.. 20  L2 Reg.. 1e-05
maxlen... 150    heads.... 1    dropout... 0.4   lr,... 0.0007  emb size... 55 min len.. 20  No L2 Reg
maxlen... 150    heads.... 1    dropout... 0.4   lr,... 0.0007  emb size... 50 min len.. 20
...
f.close()
'''
print('maxlen...', args.maxlen,'   heads....',args.num_heads,'   dropout...',args.dropout_rate,'  lr,...', args.lr,' emb size...', args.hidden_units, 'min len..', args.min_seq,' L2 Reg..', args.l2_emb,'Seed 42')
    ...
    for step in  range (0, int(num_batch) ):
        ...
        loss, _ = sess.run([ model.final_loss, model.train_op],
                                {model.u: u, model.input_seq: seq, model.pos: pos, model.neg: neg, model.buy_seq_mask:buy_seq_mask, model.cart_seq_mask: cart_seq_mask,
                                 ...
                                 model.Aug_fav_seq_mask: Aug_fav_seq_mask, model.Aug_click_seq_mask: Aug_click_seq_mask, model.labels: labels, model.epoch: epoch })
