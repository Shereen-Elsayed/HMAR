...
tstInt = None
with open('./Datasets/Tianchi_tst_int', 'rb') as fs:
...
def evaluate_valid(model, dataset, args, sess, Beh, epoch):
    ...
    for u in users:
        ...
        predictions = -model.predict(sess, [u], [seq], item_idx, [seq_cxt],testitemscxt, [buy_seq_mask], [cart_seq_mask], [fav_seq_mask], [click_seq_mask] )
        predictions = predictions[0]
...
def Model_pro(in_seq, mask, seq_cxt, args, buy_seq_mask, cart_seq_mask, fav_seq_mask, click_seq_mask,resweight_buy, resweight_cart, resweight_fav, resweight_click, is_training , reuse=None):
        ...
        #cxt
        seq = tf.compat.v1.layers.dense(inputs= seq, units=args.hidden_units,activation=None, kernel_initializer=tf.random_normal_initializer(stddev=0.01) , name="feat_emb", reuse=reuse)
        ...
        # ================== attention between all buy items ====================
        fav_seq_mask = tf.tile(tf.expand_dims(fav_seq_mask, -1), [1, 1, args.hidden_units]) # change the mask shape to be (batch, maxlen, hidden emb)
        ...
        for i in range(args.num_blocks):
            with tf.compat.v1.variable_scope("fav_num_blocks_%d" % i):
                ...
                fav_seq = feedforward(fav_seq, num_units=[args.hidden_units, args.hidden_units],
                                       dropout_rate=args.dropout_rate, is_training=is_training, scope="fav_self_attention", reuse=reuse)
...
class Model():
    def __init__(self, usernum, itemnum, args, reuse=None):
        ...
        self.seq  = Model_pro(in_seq, mask, self.seq_cxt, args, self.buy_seq_mask, self.cart_seq_mask, self.fav_seq_mask, self.click_seq_mask,self.resweight_buy,
                                                self.resweight_cart, self.resweight_fav, self.resweight_click, self.is_training , reuse=None)
        # ===================== Augmentation ==========================
        ...
        self.Aug_seq  = Model_pro(Aug_in_seq, Aug_mask, self.Aug_seq_cxt, args, self.Aug_buy_seq_mask, self.Aug_cart_seq_mask, self.Aug_fav_seq_mask, self.Aug_click_seq_mask,self.resweight_buy,
                                                self.resweight_cart, self.resweight_fav, self.resweight_click, self.is_training , reuse=True)
        ...
        trgt_cxt_emb = tf.compat.v1.layers.dense(inputs=self.pos_cxt , units=args.hidden_units,activation=None, reuse=True, kernel_initializer=tf.random_normal_initializer(stddev=0.01) , name="cxt_emb")
        ...
        #cxt
        pos_emb_in = tf.compat.v1.layers.dense(inputs=pos_emb, reuse=True, units=args.hidden_units,activation=None, kernel_initializer=tf.random_normal_initializer(stddev=0.01) , name="feat_emb")
        neg_emb_in = tf.compat.v1.layers.dense(inputs=neg_emb, reuse=True, units=args.hidden_units,activation=None, kernel_initializer=tf.random_normal_initializer(stddev=0.01) , name="feat_emb")
        ...
        self.classification_pred = tf.compat.v1.layers.dense(inputs=classification_pred1 , units=4 ,activation=tf.nn.relu, reuse=False, kernel_initializer=tf.random_normal_initializer(stddev=0.01) , kernel_regularizer = tf.keras.regularizers.l2(args.l2_emb_class) ,name="classification")
    ...
    def predict(self, sess, u, seq, item_idx, seq_cxt, test_item_cxt, buy_seq_mask, cart_seq_mask, fav_seq_mask, click_seq_mask):
        return sess.run(self.test_logits, {self.u: u, self.input_seq: seq, self.test_item: item_idx, self.is_training: False, self.seq_cxt:seq_cxt, self.test_item_cxt:test_item_cxt, \
                        model.buy_seq_mask: buy_seq_mask, model.cart_seq_mask: cart_seq_mask, model.fav_seq_mask: fav_seq_mask, model.click_seq_mask: click_seq_mask})
...
def sample_function(user_train, Beh, Beh_w, Behaviors, usernum, itemnum, batch_size, maxlen, result_queue, SEED):
    def sample():
        ...
        return (user, seq, pos, neg, seq_cxt, pos_cxt, pos_weight, neg_weight, buy_seq_mask, cart_seq_mask, fav_seq_mask, click_seq_mask , recency ,\
...
'''
maxlen... 100    heads.... 1    dropout... 0.6   lr,... 0.0005  emb size... 40 min len.. 10
maxlen... 100    heads.... 1    dropout... 0.6   lr,... 0.0005  emb size... 40 min len.. 10
 Best params
 maxlen... 100    heads.... 1    dropout... 0.55   lr,... 0.0005  emb size... 40 min len.. 10
 augmentation only crop 0.2 and mask 0.4 .... loss weight 0.01
...
f.close()
'''
print('maxlen...', args.maxlen,'   heads....',args.num_heads,'   dropout...',args.dropout_rate,'  lr,...', args.lr,' emb size...', args.hidden_units, 'min len..', args.min_seq)
...
sampler = WarpSampler(user_train, Beh, Beh_w,Behaviors, usernum, itemnum, batch_size=args.batch_size, maxlen=args.maxlen, n_workers=1)
...
for epoch in range(1, args.num_epochs + 1):
    ...
    for step in  range (0, int(num_batch) ):
        ...
        loss, _,class_loss, classification_pred = sess.run([ model.final_loss, model.train_op, model.classification_loss, model.classification_pred],
                                {model.u: u, model.input_seq: seq, model.pos: pos, model.neg: neg, model.buy_seq_mask:buy_seq_mask, model.cart_seq_mask: cart_seq_mask,
                                 model.fav_seq_mask: fav_seq_mask, model.click_seq_mask: click_seq_mask, model.is_training: True, model.seq_cxt:seq_cxt, model.pos_cxt:pos_cxt, model.pos_weight:pos_weight,
                                 ...
                                 model.Aug_fav_seq_mask: Aug_fav_seq_mask, model.Aug_click_seq_mask: Aug_click_seq_mask , model.labels: labels})
